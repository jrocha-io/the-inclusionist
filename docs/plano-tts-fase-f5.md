...
## Decisões pendentes (José)
- **D1 — Hospedagem do modelo** (o tablet Positivo pode estar **sem internet** na escola):
  - (a) **Embutir** a voz low (~20 MB) no precache do PWA → instala offline, mas **+20 MB** no install;
  - (b) **Servidor LAN** da escola (casa com o pilar LAN + store-and-forward) → sem inchar o app;
  - (c) **Lazy-fetch** de uma CDN (HuggingFace) no 1º uso → precisa de internet uma vez.
- **D2 — Threads WASM:** ONNX Runtime Web fica bem mais rápido com **SIMD + multithread**, que exigem **COOP/COEP** (cross-origin isolation) no servidor. No Positivo/Chromebook, ou o servidor manda esses cabeçalhos, ou caímos no **single-thread** (mais lento). Confirmar o que o deploy permite.
- **D3 — Voz feminina pt-BR:** aceitar a masculina `faber` por ora, ou buscar/treinar uma feminina?
- **D4 — i18n:** confirmar que Kitten/Kokoro entram **só** nos builds de inglês/outros idiomas (não pt-BR).
