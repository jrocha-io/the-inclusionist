...
const DICTS = { pt };               // dicionários já carregados (pt embutido)
...
export function t(key, params) {
  let s = (dict && dict[key] != null) ? dict[key] : (pt[key] != null ? pt[key] : key);
...
function pickDefault() {
  try { const saved = localStorage.getItem(STORE_KEY); if (saved && AVAILABLE.includes(saved)) return saved; } catch (e) {}
...
export default i18n;
if (typeof window !== 'undefined') window.__i18n = i18n; // exposto p/ teste/preview
